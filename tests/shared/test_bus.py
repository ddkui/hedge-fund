...
async def test_persistent_bus_publish_uses_xadd():
    ...
    with patch("shared.bus.redis.asyncio.from_url", return_value=mock_redis):
        ...
        await bus.connect()
        msg_id = await bus.publish("signals.quant", {"symbol": "AAPL"})
        mock_redis.xadd.assert_called_once()
        assert msg_id == "1234567890-0"


@pytest.mark.asyncio
async def test_persistent_bus_subscribe_acks_messages():
    mock_redis = AsyncMock()
    mock_redis.xgroup_create = AsyncMock(side_effect=Exception("BUSYGROUP"))
    payload = json.dumps({"symbol": "ETH"})
    mock_redis.xreadgroup = AsyncMock(side_effect=[
        [("signals.quant", [("1234-0", {"data": payload})])],
        asyncio.CancelledError(),
    ])
    mock_redis.xack = AsyncMock()

    with patch("shared.bus.redis.asyncio.from_url", return_value=mock_redis):
        bus = RedisPersistentBus("redis://localhost:6379/0", consumer_group="test-group")
        await bus.connect()
        messages = []
        try:
            async for msg in bus.subscribe("signals.quant"):
        ...
        assert messages[0]["symbol"] == "ETH"
        mock_redis.xack.assert_called_once_with("signals.quant", "test-group", "1234-0")


@pytest.mark.asyncio
async def test_persistent_bus_creates_group_on_first_use():
    mock_redis = AsyncMock()
    ...
    with patch("shared.bus.redis.asyncio.from_url", return_value=mock_redis):
        ...
        await bus.connect()
        try:
            async for _ in bus.subscribe("signals.quant"):
                break
        except (asyncio.CancelledError, StopAsyncIteration):
            pass
